...
impl<'a> DeployExecutor<'a> {
    ...
    pub(crate) async fn ensure_manifest_bindings(
        ...
        for db in &manifest.database {
            self.ensure_object_and_binding("db", db, &manifest.worker.name)
                .await?;
        }
        for kv in &manifest.kv {
            self.ensure_object_and_binding("kv", kv, &manifest.worker.name)
                .await?;
        }
        Ok(())
    }
    ...
    async fn ensure_object_and_binding(
        ...
        let object_name = binding
            ...
            .to_owned();
        progress(self.quiet, format!("ensuring {kind}/{object_name}"));
        let _ = self
            .client
