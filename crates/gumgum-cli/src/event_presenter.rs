use gumgum_core::GumgumEvent;

pub(crate) fn typed_events_for_report(report: &gumgum_api::EventsReport) -> Vec<GumgumEvent> {
...
pub(crate) fn print_events_json_lines(events: impl IntoIterator<Item = GumgumEvent>) {
    for event in events {
        println!(
            "{}",
...
pub(crate) fn event_line(event: &GumgumEvent) -> String {
    match event {
