...
- `ReconcileEvent::{target, action, message, created_at}` and `NewReconcileEvent`: event-log records. `target`/`action` are stable display/filter keys, `message` is user-facing text, and `created_at` is SQLite text output.
