use gumgum_core::{ErrorCode, GumgumError, Result, Subsystem};
...
mod tests {
    ...
    #[test]
    fn validates_workspace_manifest() {
        let raw = r#"
            [workspace]
            name = "peekaboo"
            members = ["api"]
        "#;
        let report = validate_str(raw, "gumgum.toml").unwrap();
        assert!(report.ok);
        assert!(matches!(report.manifest_kind, ManifestKind::Workspace));
    }
    ...
    fn validates_worker_manifest() {
        ...
        assert!(report.ok);
        assert!(matches!(report.manifest_kind, ManifestKind::Worker));
    }
}
