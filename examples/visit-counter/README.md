...
- Background worker consumes queue events and writes normalized rows into Postgres.
- OpenTelemetry traces span Caddy → API → Redis/MinIO/Redpanda logical operations → worker → MinIO/Postgres logical operations.
...
- current validated daemon path includes rolling revision-suffixed app containers and the platform observability stack
- the full product path is proven with core providers running: Postgres, Redis, MinIO, Redpanda
...
- prod is served at `https://kava.fund/`; preview is served at `https://visit-counter.leostera.dev/`
...
curl -k --resolve kava.fund:443:192.168.0.3 \
  https://kava.fund/
curl -k --resolve visit-counter.leostera.dev:443:192.168.0.3 \
  https://visit-counter.leostera.dev/
```
...
curl https://kava.fund/
curl https://visit-counter.leostera.dev/
gumgum events
gumgum logs api
gumgum logs worker
...
route/kava.fund routes_to worker/visit-counter-api (prod)
route/visit-counter.leostera.dev routes_to worker/visit-counter-api (preview)
```
